      ...
      <div class="col-md-4">
        <div class="content-decription-pago">
          ...
          <div v-if="weight > 1" class="precio-detail">
            <p class="medium-fs">
              Costo de envío: S.
              {{ (weight * (pricePerPound * dolarPrice)).toFixed(2) }}
            </p>
          </div>
        ...
        <div v-for="(variation, index) in variations" v-bind:key="index">
          ...
          <ul class="list-group">
            <li
              ...
              v-for="(value, indexValues) in variation.values"
              v-bind:key="indexValues"
            >
              <a
                v-if="value.dpUrl"
                class="cursor-pointer"
                @click="
                  fetch('https://www.amazon.com' + value.dpUrl, searchType)
                "
                >{{ value.value }}</a
              >
            </li>
...
export default {
  ...
  methods: {
    ...
    calculateTaxes(res) {
      ...
      this.total =
        this.price + this.price * this.earnPercentage + weightTax + priceTax;
    },
    checkAvailability() {
